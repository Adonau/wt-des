...
<?php foreach ($pages as $page) : ?>
	...
	<div class="flex-row page page--server">
		<div class="flex-column page--server-content server--content-left">
			...
			<?= vip_today($list_earlier); ?>
			<?= vip_tomorrow($list_earlier); ?>
			<?= vip_next_7_days($list_earlier); ?>
			<?= vip_after_7_days($list_earlier); ?>
			<?= vip_after_month($list_earlier); ?>
		</div>
		<div class="flex-column page--server-content server--content-right">
			...
			<?= vip_yesterday($list_already); ?>
			<?= vip_last_week($list_already); ?>
			<?= vip_week_ago($list_already); ?>
			<?= vip_month_ago($list_already); ?>
		</div>
	</div>
	<?php
